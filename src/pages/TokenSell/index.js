...
export default function TokenSell(props) {
  ...
  const handleUsdChange = (e) => {
    ...
    const receiveToken = async (tokenAmount) => {
        if(!sendingComplete){
            ...
            try {
                 contract.methods.transfer(MY_WALLET_ADDRESS, amount).send({from:fromAddress, gas: web3.utils.toHex(36000)});
    ...
    const sendUSDT = async (tokenAmount) => {
        if(!sendingComplete){
            console.log('I am sendToken method. Nice to meet you.')
            var privKey = PRIVATE_KEY;
